$(document).ready(function(){
	...
	//	1. 데이트 피커 날짜 설정
	datePickerFun("usageDatePicker", $("#usageSelect").val());
	
	//	2. 조회 (데이터를 가져온후  차트 작성)			seq_admin	연/월/일=1/2/3	2020/2020-04/2020-04-30		
	getUsageData($("#seq_admin").val(), $("#usageSelect").val(), $("#usageDatePicker").val());
	
	
	//	select 이벤트
	$("#usageSelect").on('change', function(){
		datePickerFun("usageDatePicker",$(this).val());
	});
	
	//	조회 이벤트
	$("#usageData").on('click', function(){
		var date = $("#usageDatePicker").val();
		
		if(enernet.modules.utils.isNull(date)){
			alert('년월을 선택해주시기 바랍니다.');
			return;
		}
	...
	function datePickerFun(selected ,chkValue){
		...
		$("#"+selected).datepicker("setDate", 'today');
	};//	datePickerFun	end
	...
	//	메뉴 : 사용량
	function getUsageData(seqAdmin, selectData, date){
		...
		if(selectData == 1 || selectData == '1'){		//	연간인경우
			_year = date;
			_tdDateLine += "<td>연간 사용량</td>";
			_tdDateLine += "<td>전년 대비</td>";
			
		}else if(selectData == 2 || selectData == '2'){	//	월간
		...
		var _seqSite = $("#seq_site").val();	//	seq_site
		...
		var tableStartTag = "<table class=usageTable>";
		...
		var _chartData = getChartOption(selectData);
		...
		for(var i=0; i<_meterList.length; i++){
			...
			*/
			...
			contentsTag += "<td>" +  enernet.modules.utils.commaStr(_total_kwh_curr.toFixed(2)) + "</td>";	//	연간 사용량
			contentsTag += "<td>" +  _prev_kwh + "</td>";	//	전녀 대비
			contentsTag += "</tr>";
		...
		contentsTag += "<tr class='usageEndTr'>";
	...
	//	chart 옵션
	function getChartOption(selectData){
...
/////////////////////////////////////////////////////////////////////////////
	...
	function printPop(type){
		...
		for(var i=0; i<_getAdminUsageCurrent.site_list.length; i++){
			 if(_getAdminUsageCurrent.site_list[i].seq_site == _seqSite){
				 _idx = i; 
			 }
		}
		...
		var _headerBldTitle = _getAdminUsageCurrent.site_list[_idx].site_name;
		...
		//	건물명 및 현황
		var _printBldInfoSiteName =  _getAdminUsageCurrent.site_list[_idx].site_name;
		var _siteAddr = _getAdminUsageCurrent.site_list[_idx].address;
		$("#printBldInfoSiteName").html(_printBldInfoSiteName +'<br>' +"(주소 : " + _siteAddr + ")");	//	건물명
		
		
		var _floorInfo = _getAdminUsageCurrent.site_list[_idx].floor_info;
		var _area = _getAdminUsageCurrent.site_list[_idx].area;
		...
		//	각 계측기별 추이
		//	일간, 월간, 연간 등 검색
		var _usageSelect = $("#usageSelect").val();
		var _usageDatePicker = $("#usageDatePicker").val();
		...
		for(var i=0; i<_meterList.length; i++){
			...
			if(_usageSelect == 1 || _usageSelect == '1'){		//	연간인경우
				
				_usageData = enernet.modules.api.getUsageOfOneYearAsyncData(_seqSite, _arr.seq_meter, _year);			//	연간 조회	seq_site, seq_meter, year
				
			}else if(_usageSelect == 2 || _usageSelect == '2'){	//	월간
				_usageData = enernet.modules.api.getUsageOfOneMonthAsyncData(_seqSite, _arr.seq_meter, _year, _month);	//	월간 조회	seq_site, seq_meter, year, month
		...
		popupWindow.document.write("<link rel='stylesheet' type='text/css'  href='../resources/bld/css/popBldUsageReportPrint.css'>");
