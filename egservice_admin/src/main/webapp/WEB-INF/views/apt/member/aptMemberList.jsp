										...
										<div class="col-sm-12">
											<div style="width:100%;">
												<div style="float:right;font-size:13px;">
													총 세대 ${COUNT_MAP.ho_count} &ensp;|&ensp; APP 가입 ${COUNT_MAP.joined_main_member_count}
												 	<%-- 세대 : ${COUNT_MAP.cnt_ho} / ${COUNT_MAP.tot_cnt_ho} &ensp; | &ensp; 공가 : ${COUNT_MAP.cnt_empty_ho} &ensp; | &ensp; App 가입 : ${COUNT_MAP.app_join_cnt_member} 세대 --%>
												 </div>
											</div>
												...
												<table class="table table-bordered table-hover dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;text-align: center;">
													<thead>
														<tr>
															<td width="5%">동</td>
															<td width="5%">호</td>
															<td width="10%">세대 대표</td>
															<td width="15%">전화번호</td>
															
															<td width="10%">MID</td>
															<td width="15%">세대 대표 App ID</td>
															<td width="13%">스마트폰 기종</td>
															<td width="12%">세대원 가입 수</td>
															<td width="15%">최근 로그인</td>
														</tr>
													</thead>
													...
													<tbody>
														<c:forEach items="${LIST_MEMBER}" var="LIST_MEMBER">
															<tr>
																...
																<td>
																	...
																	<c:if test="${LIST_MEMBER.candidate_phone ne null}">
																		...
																		<c:if test="${fn:length(CANDIDATE_PHONE) == 10 }">
																			<c:out value="${fn:substring(CANDIDATE_PHONE, 0, 3)}-${fn:substring(CANDIDATE_PHONE, 3, 6)}-${fn:substring(CANDIDATE_PHONE, 6, fn:length(CANDIDATE_PHONE))}"/>
																		</c:if>
																		
																		<c:if test="${fn:length(CANDIDATE_PHONE) == 11 }">
																...
																<td>
																	<c:if test="${LIST_MEMBER.device_os eq null}">
																			-
																	</c:if>
																	
																	<c:if test="${LIST_MEMBER.device_os ne null}">
																		${LIST_MEMBER.device_os}
																	</c:if>
																	
																</td>
																
																<td>
																	<c:if test="${LIST_MEMBER.family_member_count eq 0}">
																		-
																	</c:if>
																	
																	<c:if test="${LIST_MEMBER.family_member_count ne 0 }">
																	
																		${LIST_MEMBER.family_member_count}	
																	</c:if>
															...
															<!-- 	<td>
																	<input type="button" class="btn btn-outline-success btn-sm" value="변경">
																</td> -->
															</tr>
														</c:forEach>
													</tbody>
