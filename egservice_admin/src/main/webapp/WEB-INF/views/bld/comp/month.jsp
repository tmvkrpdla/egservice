...
<c:if test="${not empty sessionScope}">
	...
	<script src="${pageContext.request.contextPath}/resources/admin/bld/comp/js/bldCompMonth.js"></script>
							...
							<div class="col-xl-12">
								<div class="mb-4a page-heading-div">
									...
									<div class="page-heading-title-div">
										<span class="mb-0 text-gray-800 page-heading-title">날짜별
											비교</span> &nbsp; &nbsp; &nbsp; <span class="page-heading-navi-span"><i
											class="fas fa-fw fa-home"></i> > 계측기간 사용량 비교 > 날짜별 비교</span>
									</div>
									...
									<div class="card-body">
										<div style="width: 100%; display: inline-block;">
											<form id="searchForm" name="searchForm" method="get"
												action="<c:url value='../bld/compMonth'/>">
												<div style="width: 100px; float: left;">
													<span style="display: inline-block; height: 21px;"></span>월
													선택
												</div>

												<div style="width: 200px; float: left;">
													<input class="custom-date custom-date-sm" id="monthTarget"
														name="monthTarget" type="text" style="width: 95%;"
														value="${MONTHTARGET}">
												</div>
										...
										<div class="width:100%;display: inline-block;">
											<span> ${MONTHTARGET} </span> &emsp; <span> 최고 기온 : <c:set
													value="${WEATHERMONTH.avg_temp_max}" var="AVG_TEMP_MAX" />

												<%-- <c:if test="${AVG_TEMP_MAX eq 'NaN' }">
                  											-
                  										</c:if>
                  										<c:if test="${AVG_TEMP_MAX ne 'NaN' }"> --%>
												<c:if test="${AVG_TEMP_MAX < 0}"> - </c:if> <c:if
													test="${AVG_TEMP_MAX > 0}">
													<fmt:formatNumber value="${AVG_TEMP_MAX}"
														pattern="#,###.00" />  ℃  </c:if> <%-- </c:if> --%>

											</span> &emsp; <span> 최저 기온 : <c:set
													value="${WEATHERMONTH.avg_temp_min}" var="AVG_TEMP_MIN" />
												...
												<c:if test="${AVG_TEMP_MIN < 0}"> - </c:if> <c:if
													test="${AVG_TEMP_MIN > 0}">
													<fmt:formatNumber value="${AVG_TEMP_MIN}"
														pattern="#,###.00" /> ℃ </c:if> <%-- </c:if> --%>


											</span> &emsp; <span> 평균 기온 : <c:set
													value="${WEATHERMONTH.avg_temp_avg}" var="AVG_TEMP_AVG" />
												<%-- 
												...
												<c:if test="${AVG_TEMP_AVG < 0}"> - </c:if> <c:if
													test="${AVG_TEMP_AVG > 0}">
													<fmt:formatNumber value="${AVG_TEMP_AVG}"
														pattern="#,###.00" /> ℃ </c:if> <%-- </c:if> --%>
							...
							<div class="col-xl-12">
								<div class="card shadow mb-4">
												...
												<div class="card shadow mb-4">
													<div class="card-body">
														...
														<div class="lightbulb-text-style">
															...
															<div>- 가장 많은 전기 사용량을 어느 계측기에서 발생하는지 찾아봅니다.</div>

															<div>- 휴무일에도 전기 사용량이 많은 계측기가 있는지 찾아봅니다.</div>

															<div>- 상세한 사용량 패턴 분석이 필요한 계측기가 발견되면 "계측기별 사용량 분석"
																페이지를 이용합니다.</div>
								 ...
								 --%>
									...
									<c:forEach items="${USAGE_METER_MONTH_LIST}"
										var="USAGE_METER_MONTH_LIST" varStatus="STATUS">
										<div class="" style="flex: 1 1 auto; padding: 0 1.25rem;">
											<div style="width: 100%;">-
												${USAGE_METER_MONTH_LIST.descr }</div>
											...
											<div style="width: 100%;">
												<div style="width: 50%; float: left">
													...
													<table
														...
														<c:forEach items="${WEATHERMONTH.list_weather}"
															var="LIST_WEATHER" varStatus="ST">
															<c:if test="${ST.index <15 }">
																<tr>
																	<td>${LIST_WEATHER.unit}일</td>
																	<td>${LIST_WEATHER.text_day_of_week}</td>
																	<td><c:if
																			test="${USAGE_METER_MONTH_LIST.list_usage[ST.index].text_usage eq null}">
															-
														</c:if> <c:if
																			test="${USAGE_METER_MONTH_LIST.list_usage[ST.index].text_usage ne null}">
															${USAGE_METER_MONTH_LIST.list_usage[ST.index].text_usage}
														</c:if></td>
																	<td><c:if test="${LIST_WEATHER.weather_code < 0}">
															-
														</c:if> <c:if test="${LIST_WEATHER.weather_code > 0}">
																			<img
																				src="${pageContext.request.contextPath}/${LIST_WEATHER.text_weather_code}"
																				style="width: 20px;">
																		</c:if></td>
																	<td><c:if test="${LIST_WEATHER.temp_max < 0}">
															-
														</c:if> <c:if test="${LIST_WEATHER.temp_max > 0}">
															${LIST_WEATHER.temp_max}
														</c:if></td>
																	<td><c:if test="${LIST_WEATHER.temp_min < 0}">
															-
														</c:if> <c:if test="${LIST_WEATHER.temp_min > 0}">
															${LIST_WEATHER.temp_min}
														</c:if></td>
																	<td><c:if test="${LIST_WEATHER.temp_avg < 0}">
															-
														</c:if> <c:if test="${LIST_WEATHER.temp_avg > 0}">
